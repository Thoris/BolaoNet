...
namespace BolaoNet.Domain.Services.Boloes
{
        ...
        Interfaces.Services.Boloes.IBolaoRegraService
    {
        ...
        public IList<Entities.Boloes.BolaoRegra> GetRegrasBolao(Entities.Boloes.Bolao bolao)
        {
            ...
            IList<Entities.Boloes.BolaoRegra> res =
                Dao.GetRegrasBolao(base.CurrentUserName, DateTime.Now, bolao);

            if (IsSaveLog)
            {
                _logging.Debug(this, GetMessageTotalTime("Carregamento das regras do bolão [" + bolao.Nome + "] total: " + res.Count));
            }
