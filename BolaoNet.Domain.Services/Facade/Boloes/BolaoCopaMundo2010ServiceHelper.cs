...
{
    public class BolaoCopaMundo2010ServiceHelper : IBolaoFacadeService
    {
        #region Constants
        ...
        public BolaoCopaMundo2010ServiceHelper(Interfaces.Services.Boloes.IApostaExtraService apostaExtraService,
        ...
        public Domain.Entities.Boloes.Bolao CreateBolao(Domain.Entities.Campeonatos.Campeonato campeonato)
        {
            ...
            StoreData<Domain.Entities.Boloes.BolaoRegra>(_bolaoRegraService, new Domain.Entities.Boloes.BolaoRegra(bolao.Nome, c++) { Descricao = "Premiação para o terceiro lugar: 10% do dinheiro arrecadado" });
            StoreData<Domain.Entities.Boloes.BolaoRegra>(_bolaoRegraService, new Domain.Entities.Boloes.BolaoRegra(bolao.Nome, c++) { Descricao = "havendo dois primeiros lugares, 45% para cada e o próximo leva 10%; havendo empate no segundo lugar, divide-se entre eles os 30% (se dois empatarem 15% para cada)" });
            StoreData<Domain.Entities.Boloes.BolaoRegra>(_bolaoRegraService, new Domain.Entities.Boloes.BolaoRegra(bolao.Nome, c++) { Descricao = "Se caso a aposta não foi realizado, será considerado o placar de 0x0" });
            StoreData<Domain.Entities.Boloes.BolaoRegra>(_bolaoRegraService, new Domain.Entities.Boloes.BolaoRegra(bolao.Nome, c++) { Descricao = "A taxa de participação do bolão será de 20 reais" });
            ...
            StoreData<Domain.Entities.Boloes.BolaoRegra>(_bolaoRegraService, new Domain.Entities.Boloes.BolaoRegra(bolao.Nome, c++) { Descricao = "Com relação ao resultado dos 4 primeiros: Valerá apenas 15 pontos cada acerto." });
        ...
        public Domain.Entities.Users.User CreateUserApostasFixo(string nomeBolao, string nomeCampeonato, string userName, string email, int time1, int time2)
        {
            ...
            //Domain.Entities.Boloes.Bolao bolaoLoaded = _bolaoService.Load(bolao);
            //Domain.Entities.Campeonatos.Campeonato campeonato = _campeonatoService.Load(new Domain.Entities.Campeonatos.Campeonato(bolaoLoaded.NomeCampeonato));
        ...
        public bool CreateUserApostas(Domain.Entities.Users.User user, Domain.Entities.Boloes.Bolao bolao, IList<Domain.Entities.Boloes.JogoUsuario> jogos)
        {
            ...
            if (_userService.Load(user) == null)
            {
                _userFacadeService.CreateUser(user, roles);
                _userFacadeService.SendActivationCode(user);

                Domain.Entities.Users.User loadedUser = _userService.Load(user);
                activationCode = loadedUser.ActivateKey;

                _userFacadeService.ActivateUser(user, activationCode);
            }
            ...
            if (_bolaoMembroService.Load(membro) == null)
            {
                _bolaoMembroService.Insert(membro);
            }
            ...
            for (int c = 0; c < jogos.Count; c++)
            {
                ...
                _jogoUsuarioService.ProcessAposta(bolao, user, jogo, 1, (int)jogos[c].ApostaTime1, (int)jogos[c].ApostaTime2,
