...
namespace BolaoNet.Domain.Services.Facade.Boloes
{
    public class BolaoCopaMundo2026ServiceHelper : IBolaoFacadeService
    {
        ...
        public Domain.Entities.Boloes.Bolao CreateBolao(Domain.Entities.Campeonatos.Campeonato campeonato)
        {
            ...
            StoreData<Domain.Entities.Boloes.BolaoRegra>(_bolaoRegraService, new Domain.Entities.Boloes.BolaoRegra(bolao.Nome, c++) { Descricao = "   * Em caso de empate nas posições premiadas, o valor correspondente será dividido proporcionalmente entre os participantes empatados. Exemplo: caso haja empate em 1º lugar entre dois participantes, cada um receberá 45% da premiação, e o participante subsequente receberá a premiação referente ao 3º lugar (9%). Em caso de empate em 2º lugar entre dois participantes, os 29% restantes serão divididos igualmente, resultando em 14,5% para cada participante." });
            ...
            StoreData<Domain.Entities.Boloes.BolaoRegra>(_bolaoRegraService, new Domain.Entities.Boloes.BolaoRegra(bolao.Nome, c++) { Descricao = "As partidas da Seleção Brasileira terão pontuação dobrada. Dessa forma, os critérios passarão a valer 20, 10, 8, 6 e 2 pontos, respectivamente. Exemplo: ao acertar exatamente o placar de uma partida do Brasil, o participante receberá 20 pontos ao invés de 10." });
            ...
            StoreData<Domain.Entities.Boloes.BolaoRegra>(_bolaoRegraService, new Domain.Entities.Boloes.BolaoRegra(bolao.Nome, c++) { Descricao = "Para fins de pontuação na fase eliminatória, será considerado válido o resultado correspondente ao confronto oficial da competição, independentemente das seleções inicialmente exibidas na projeção da aposta. Exemplo: caso a aposta apresente Brasil x Chile, mas o confronto real venha a ser Portugal x Chile, a pontuação será calculada normalmente com base no resultado efetivo do jogo realizado." });
            StoreData<Domain.Entities.Boloes.BolaoRegra>(_bolaoRegraService, new Domain.Entities.Boloes.BolaoRegra(bolao.Nome, c++) { Descricao = "Na fase eliminatória, a regra de pontuação dobrada será aplicada exclusivamente às partidas reais da Seleção Brasileira, e não aos confrontos projetados automaticamente pelo sistema durante o preenchimento das apostas." });
            StoreData<Domain.Entities.Boloes.BolaoRegra>(_bolaoRegraService, new Domain.Entities.Boloes.BolaoRegra(bolao.Nome, c++) { Descricao = "Os confrontos da fase eliminatória serão definidos com base nas posições classificatórias dos grupos. Assim, o participante estará apostando, por exemplo, no 1º colocado do Grupo A contra o 2º colocado do Grupo B, independentemente das seleções efetivamente classificadas." });
            StoreData<Domain.Entities.Boloes.BolaoRegra>(_bolaoRegraService, new Domain.Entities.Boloes.BolaoRegra(bolao.Nome, c++) { Descricao = "Em situações excepcionais envolvendo desistência, exclusão, suspensão ou substituição de seleções participantes da competição após o registro das apostas, todas as apostas diretamente associadas à seleção afetada serão automaticamente consideradas como pontuação máxima de 10 pontos para os participantes. Exemplo: caso uma seleção como o Irã venha a desistir ou seja retirada da competição, todas as apostas vinculadas aos jogos dessa seleção receberão pontuação integral equivalente ao acerto exato do placar." });
            StoreData<Domain.Entities.Boloes.BolaoRegra>(_bolaoRegraService, new Domain.Entities.Boloes.BolaoRegra(bolao.Nome, c++) { Descricao = "Após o encerramento do prazo oficial para realização das apostas, será encaminhado a todos os participantes um arquivo em formato PDF contendo a consolidação completa das apostas registradas por todos os participantes do bolão. Cada participante terá o prazo de até 24 (vinte e quatro) horas, contadas a partir do envio do arquivo, para solicitar eventuais correções, ajustes ou reportar inconsistências relacionadas às apostas registradas. Decorrido este prazo sem manifestação formal do participante, todas as apostas constantes no documento serão consideradas corretas, validadas e oficialmente aceitas para fins de apuração da competição." });
            StoreData<Domain.Entities.Boloes.BolaoRegra>(_bolaoRegraService, new Domain.Entities.Boloes.BolaoRegra(bolao.Nome, c++) { Descricao = "Ao término da competição, será encaminhado por e-mail aos participantes um novo arquivo consolidado em formato PDF contendo todas as apostas realizadas, as pontuações obtidas em cada fase e a classificação final do bolão. Após o envio do referido documento, os participantes terão igualmente o prazo de até 24 (vinte e quatro) horas para apresentação de eventuais questionamentos, solicitações de revisão, ajustes ou reclamações relacionadas à apuração dos resultados. Após o término deste prazo, os resultados apresentados serão considerados definitivos, homologados e aceitos por todos os participantes." });
            #endregion
