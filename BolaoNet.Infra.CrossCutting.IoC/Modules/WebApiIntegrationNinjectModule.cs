...
namespace BolaoNet.Infra.CrossCutting.IoC.Modules
{
    ...
    public class WebApiIntegrationNinjectModule : NinjectModule
    {
        ...
        private string GetUrlWebApi()
        {
            return System.Configuration.ConfigurationManager.AppSettings["UrlWebApi"];
        }

        public override void Load()
        {
            //string url = "http://localhost:9586/";
            string url = GetUrlWebApi();
            ...
            Bind<Domain.Interfaces.Services.Boloes.IBolaoPremiacaoService>().To<WebApi.Integration.Boloes.BolaoPremiacaoIntegration>()
                ...
                .WithConstructorArgument("url", url)
                .WithConstructorArgument("token", token);
            Bind<Domain.Interfaces.Services.Boloes.IBolaoAcertoTimePontoService>().To<WebApi.Integration.Boloes.BolaoAcertoTimePontoIntegration>()
                .WhenInjectedExactlyInto(typeof(Application.Boloes.BolaoAcertoTimePontoApp))
                .WithConstructorArgument("url", url)
                .WithConstructorArgument("token", token);
