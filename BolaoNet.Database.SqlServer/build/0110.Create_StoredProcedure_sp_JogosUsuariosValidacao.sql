...
BEGIN
	...
	DECLARE @NomeTimeResult1		varchar(150)
	DECLARE @NomeTimeResult2		varchar(150)
	...
	DECLARE @CountPontosAcertoTime	int
	...
	SET @PontosCheio			= 0
	SET @PontosAcertoTime		= 0		-- Total de pontos de acerto de time
	...
	SET @CountCheio				= 0
	SET @CountPontosAcertoTime	= 0		-- Contador de acerto de times
	...
	DECLARE curJogosUsuarios CURSOR FOR
		SELECT	NomeBolao, UserName, ApostaTime1, ApostaTime2, NomeTimeResult1, NomeTimeResult2
		  FROM JogosUsuarios
		 WHERE JogoID			= @IDJogo
	...
	-- Executando o cursor
	FETCH NEXT FROM curJogosUsuarios INTO @NomeBolao, @UserName, @Aposta1, @Aposta2, @NomeTimeResult1, @NomeTimeResult2
	...
	BEGIN
		...
		BEGIN			
			...
			SELECT TOP 1 @MultiploTime = MultiploTime
			...
			-- Buscando os pontos adicionais por acerto de time
			SELECT @PontosAcertoTime = ISNULL(Pontos, 0)
			  FROM BoloesAcertoTimePontos
			 WHERE NomeCampeonato	= @NomeCampeonato
			   AND NomeBolao		= @NomeBolao
			   AND JogoId			= @IdJogo

			PRINT 'Pontos para acerto de time: ' + CONVERT(VARCHAR, @PontosAcertoTime)											
		...
		UPDATE JogosUsuarios
			SET	Valido					= 1,
				...
				PontosAcertoTime		= @CountPontosAcertoTime
			WHERE JogoId		= @IdJogo
			AND NomeBolao		= @NomeBolao
			AND NomeCampeonato	= @NomeCampeonato			   
		...
		SELECT	a.UserName, 
				...
				ISNULL(SUM(CONVERT(INT, a.PontosAcertoTime)),0),
				@CurrentLogin, @CurrentDateTime, @CurrentLogin, @CurrentDateTime, 0
				--b.IDJogo							
			FROM JogosUsuarios a
			INNER JOIN Jogos b
			   ON a.NomeCampeonato		= b.NomeCampeonato
			  AND a.JogoID				= b.JogoID
