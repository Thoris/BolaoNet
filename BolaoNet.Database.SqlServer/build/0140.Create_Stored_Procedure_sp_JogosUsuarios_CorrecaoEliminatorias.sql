...
BEGIN
	...
	SELECT jogo.Jogoid, jogo.Rodada, jogo.PendenteTime1Ganhador, jogo.PendenteIdTime1, jogo.PendenteTime2Ganhador, jogo.PendenteIdTime2
	  FROM JogosUsuarios usu
	 INNER JOIN Jogos jogo
		ON usu.NomeCampeonato	= jogo.NomeCampeonato
	   AND usu.JogoId			= jogo.JogoId
	 WHERE usu.NomeCampeonato	= @NomeCampeonato
	   AND usu.NomeBolao		= @NomeBolao
	   AND usu.UserName			= @UserName
	   AND (jogo.PendenteIdTime1 > 0 OR jogo.PendenteIdTime2 > 0)
		...
		BEGIN
			...
			IF (EXISTS (SELECT * 
						 ...
						 WHERE NomeBolao		= @NomeBolao 
						   ...
						   AND JogoId			= @CurPendenteTime1JogoId))
			BEGIN
			...
			IF (EXISTS (SELECT * 
						 ...
						 WHERE NomeBolao		= @NomeBolao 
						   AND NomeCampeonato	= @NomeCampeonato
						   AND UserName			= @UserName
						   AND JogoId			= @CurPendenteTime2JogoId))
			BEGIN
	...
	SELECT jogo.JogoId, usu.ApostaTime1, usu.ApostaTime2, usu.NomeTimeResult1, usu.NomeTimeResult2, usu.Ganhador, jogo.NomeGrupo
	  FROM JogosUsuarios usu
	 INNER JOIN Jogos jogo
		ON usu.NomeCampeonato	= jogo.NomeCampeonato
	   AND usu.JogoId			= jogo.JogoId
